...
public class PrefixTree {
    ...
    prefix(current, new StringBuilder(prefix), result);
    return result;
}

    private void prefix(TreeNode node, StringBuilder pre, ArrayList<String> result){
        if(node.isWord){
            result.add(pre.toString());
        }
        for (Map.Entry<Character,TreeNode> entry : node.children.entrySet()) {
            pre.append(entry.getKey());
            prefix(entry.getValue(), pre, result);
